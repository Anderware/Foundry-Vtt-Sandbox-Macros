...
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 
//
function Setting_WeaponGroupKey()          {return 'WeaponsGroup';  }// this must be specified according to your groups key
function Setting_WeaponAttackRoll()          {return 'WeaponAttackRoll';  }// this must be specified according to your property key
              ...
              if (citem.groups.length>0){                         
                ...
                for(let iGroup=0;iGroup<citem.groups.length;iGroup++){  
                  ...
                  if(citem.groups[iGroup].ikey==weaponSelectedGroupKey){ 
                    ...
                    lielement.innerHTML='<label class="${thisMacroName()}_hbo" onclick="${thisMacroName()}_rollTokencItem(' + ${thisMacroName()}_singleQuoted(actorId) +','+ ${thisMacroName()}_singleQuoted(citem.name) + ');">' + citem.name + '</label> ';
