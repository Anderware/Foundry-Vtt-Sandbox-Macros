...
//  if(token!=null){
//    simg=token.document.texture.src;
//    sname=token.document.texture.src;
//  }           
//}
//ChatMessage.create({
...
// 2023-02-23 2.1.0    Ramses800            Updated for Foundry 10.291/Sandbox 0.14.0 NOT COMPATIBLE WITH EARLIER VERSIONS  
// ****************************************************************                                                                                                                                         
function macroTitle() {
  ...
  <script>
   ...
   // support functions
    ...
    function ${thisMacroName()}_rollActorProperty(actorId,sPropertyId,useCanvastokens=0 ){
      ...
      if(actor!=null){             
        let actorattributes = actor.system.attributes;
        let rollexp;
        let rollname;
        let rollid = [];
    ...
    function ${thisMacroName()}_rollActorcItemProperty(actorId,citemname,sPropertyId,useCanvastokens=0){ 
      ...
      //console.log(actor);
      if(actor!=null){          
        let citem = actor.system.citems.find(y=>y.name == citemname);
        let property = game.items.get(sPropertyId);  
        let citemattributes = citem.attributes;
        rollexp = property.system.rollexp;
        rollname = property.system.rollname;         
        actor.rollSheetDice(rollexp,rollname,null,actor.system.attributes,citemattributes); 
      }
    }
      ...
      arrProperties.forEach(function(item){
        ...
        if (item.hasOwnProperty('columnheadertooltip')){
          ...
          if (headeritem!=''){
            ...
            headerproperty = game.items.find(y=>y.type=="property" && y.system.attKey==propkey);
            if (headerproperty!=null) { 
                  
              if (headerproperty.system.hasOwnProperty('tooltip')){ 
                tooltip= headerproperty.system.tooltip;
                if (tooltip!=''){
                  if (iscitem){
    ...
    function ${thisMacroName()}_DisplayCellContent(cell,actorId,display,roll,useCanvastokens=0){      
      ...
      try{ 
        ...
        if (display.indexOf('|')>-1){          
          ...
          citem = actor.system.citems.find(y=>y.name == citemname);                    
        }                                    
        // display label         
        if (display=='' || display.startsWith("%")){ 
          ...
          if(!citem){ 
            // actor property
            property = game.items.find(y=>y.type=="property" && y.system.attKey==roll); 
            if(property!=null) {  
              if (['simplenumeric', 'simpletext', 'list', 'checkbox','radio','badge'].indexOf(property.system.datatype) >= 0){   
                 if(actor.system.attributes.hasOwnProperty(roll)){
                  hasthis=true;
                }                       
              }
          ...
          if(!iscitem){
            property = game.items.find(y=>y.type=="property" && y.system.attKey==display);
            if (property!=null) {
              //                 
              // check if this is a value attribute  
              if (['simplenumeric', 'simpletext', 'list', 'checkbox','radio','badge'].indexOf(property.system.datatype) >= 0) {
                // check that actor has this attribute
                 if(actor.system.attributes.hasOwnProperty(display)){ 
                    if(property.system.datatype=='badge'){ 
                      slabel= actor.system.attributes[display].value + '(' + actor.system.attributes[display].max + ')';
                    }                                           
                    else if(property.system.datatype=='checkbox'){ 
                      if(actor.system.attributes[display].value==true){
                      ...
                      // add radio glyphs  
                      for(let i=1;i<=actor.system.attributes[display].max;i++){ 
                        if(actor.system.attributes[display].value>=i){  
                          if (property.system.radiotype=='C'){
                    ...
                    // use the value from targetattributes as label
                      slabel=actor.system.attributes[display].value; 
                    }                  
                 } 
                 else{
                ...
                if(property.system.labelformat='D'){                  
                  ...
                  slabel=property.system.tag;
                }                 
              } 
              
              else{ 
                // unsupported datatype
                ui.notifications.warn('Unsupported data type for property '+display + ':' + property.system.datatype );
                slabel=property.system.datatype;
              }                               
              //
            }
          }
            ...
            if (citem!=null){
              ...
              property = game.items.find(y=>y.type=="property" && y.system.attKey==citempropKey);
              if (property!=null){     
                // check if this is a value attribute   
                if (['simplenumeric', 'simpletext', 'list', 'checkbox','radio','badge'].indexOf(property.system.datatype) >= 0) { 
                  if(citem.attributes.hasOwnProperty(citempropKey)){
        ...
        if(roll==''|| slabel=='' ){
          ...
          if(roll.startsWith("?")){
            ...
            if(!iscitem){   
              ...
              property = game.items.find(y=>y.type=="property" && y.system.attKey==roll); 
              if (property!=null){    
                let rollexp = property.system.rollexp;
                if (rollexp!=null){
                  if (rollexp.length>0){                                                                                          
    ...
    function ${thisMacroName()}_ListActorsProperties() {
      ...
      if (foldername!=''){
        ...
        if (folder!=null){
          ${thisMacroName()}_foldercolor=folder.color
      ...
      if(useCanvastokens==0){  
        ...
        for(let actor of actors){
          ...
          row.insertCell(-1).outerHTML = '<td class="${thisMacroName()}_portrait_td"><img class="${thisMacroName()}_portrait_img"  src="' + actor.img +'"</img></td>';
        ...
        for(let token of tokens){            
          let row = tbody.insertRow(-1);
          row.style.lineHeight="12px";
          row.insertCell(-1).outerHTML = '<td class="${thisMacroName()}_portrait_td"><img title="Show token configuration" class="${thisMacroName()}_portrait_img ${thisMacroName()}_hbox" onclick="${thisMacroName()}_showTokenConfiguration(' + ${thisMacroName()}_singleQuoted(token.id) +',' + useCanvastokens +  ');" src="' + token.document.texture.src +'"</img></td>';
          row.insertCell(-1).outerHTML = '<td class="${thisMacroName()}_actorname_td"><label title="Show actor" class="${thisMacroName()}_hbo" onclick="${thisMacroName()}_showSheet(' + ${thisMacroName()}_singleQuoted(token.id) +',' + useCanvastokens +  ');">' + token.name +'</label></td>';
          arrProperties.forEach(function(item){
            sPropertyName=item.display;            
            let cell=row.insertCell(-1);
            cell.style.textAlign = "center";  
            ...
            ${thisMacroName()}_DisplayCellContent(cell,token.id,item.display,roll,useCanvastokens);          
          });
        }
      }
    } 
