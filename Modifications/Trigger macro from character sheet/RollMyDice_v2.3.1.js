...
let thismacro_rollname='';

// get arguments from user flag with this macro´s name
let macroargs=await game.user.getFlag('world',thismacro_macroname);  
if (macroargs!=null){
  thismacro_rollname=GetArguments(macroargs,'rollname','string');      
}
...
async function BuildTableUIs(){
  ...
  objRollTables.forEach(function(table){  
    ...
    if (macroargs!=null){  
      table.defaultnrofdice = GetArguments(macroargs,table.tablename);
    } 
  ...
  buttons: {
    ...
    yes: {
      ...
      callback: async (html) => { 
        ...
        let baseMessage = '<div style="    font-family: Tahoma, sans-serif;    font-weight: bold;    font-style: italic;    font-size: 12px;    text-align: center;    width: 100%;    margin-top: -10px;">' + thismacro_rollname + '</div>';        
